...
for csv_file in csv_files:
    ...
    if os.path.exists(output_file_path):
        skipped_files += 1
        print(f"[{file_count}/{total_files}][{timestamp}] Skipping: {csv_file} (already filtered)")
        continue
        
    # Read the CSV file into a dataframe
    df = pd.read_csv(os.path.join(input_dir, csv_file), sep=';', encoding='latin-1', low_memory=False)

    # Filtering
    df = (df.ffill()).bfill()
    filtered_df = df.loc[(df['NudzBr_1'] == 1) | (df['NudzBr_2'] == 1) | (df['Zvonec'] == 1) | (df['KolBr_1'] == 1) | (df['KolBr_2'] == 1) | (df['Sklz_Smyk'] == 1)]
